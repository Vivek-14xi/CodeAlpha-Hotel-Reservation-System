...
public class Reservation {
    ...
    private Reservation(Customer customer,Room room,LocalDate checkInDate,LocalDate checkOutDate , int numberOfGuest){
        ...
        if (numberOfGuest <1 ||numberOfGuest > room.getMaxOccupancy()){
            throw new IllegalArgumentException("Guest count must be between 1 and "+ room.getMaxOccupancy()+"for a "+room.getRoomType().getDisplayName()+"room.");
        }
    ...
    public Reservation(String reservationId, Customer customer,Room room, LocalDate  checkInDate,LocalDate checkOutDate,int numberOfGuest, ReservationStatus status, double totalAmount, String bookingDate){
    ...
    public void setCheckIn(){
        if (status!= ReservationStatus.CONFIRMED)
            throw new IllegalArgumentException("Cannot check in : reservation is"+ status);
        this.status=ReservationStatus.CHECKED_IN;
        System.out.println("Guest"+customer.getName()+ "has checked into Room "+room.getRoomNumber());
    }
    public void setCheckOut(){
        if (status!=ReservationStatus.CHECKED_IN){
            throw  new IllegalArgumentException("Cannot check out: reservation is "+status);
            }
        this.status=ReservationStatus.CHECKED_OUT;
        this.room.setAvailable(true);
        System.out.println("Guet "+customer.getName()+"has checked out from Room "+room.getRoomNumber());
    }
